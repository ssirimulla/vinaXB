...
fl weighted_terms::eval(sz t1, sz t2, fl r, fl theta) const { // intentionally not checking for cutoff
	fl acc = 0;
    VINA_FOR_IN(i, enabled_usable_terms){
		acc += weights[i] * t->usable_terms[enabled_usable_terms[i]].eval(t1, t2, r, theta);
    }
	return acc;
}
